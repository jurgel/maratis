...
MObject3d * getObject3d(unsigned int id)
{
	...
	if((id < scene->getObjectsNumber())&&(id < 10000))
		return scene->getObjectByIndex(id);
	if(id >= 10000)
	{
	    id = id -10000;
	    if(id < hud->getObjectsNumber())
	        return hud->getObjectByIndex(id);
	}
...
int getSize(lua_State * L)
{
    ...
    if((object = getObject3d(id)))
    {
        MVector3 * sMax = ((MOEntity *)object)->getBoundingBox()->getMax();
        MVector3 * sMin = ((MOEntity *)object)->getBoundingBox()->getMin();
        MVector3 size = *sMax - *sMin;
        pushFloatArray(L, size, 3);
        return 1;
    }
...
{
    lua_pushstring(L, g_currentDirectory);
    return 1;
}
...
void MScript::init(void)
{
	...
	lua_register(m_state, "isObject",    isObject);
	...
	lua_register(m_state, "getSize",     getSize);
	...
	lua_register(m_state, "getCurrentDir", getCurrentDir);
